...
class PayoffMatrix:
                ...
                if matrix.shape == (2, 3, 3):
                    ...
                    raise TypeError(
                        "Predefined should be a valid matrix where \
                                     matrix.shape == (2,3,3), e.g. a 2x2x2 \
                                     matrix"
                    )
            else:
                raise TypeError(
    ...
    def __str__(self):
        ...
        str6 = (
            ...
            f"{add_pl(self()[0][0,1])}" + " | " + f"{add_pl(self()[0][0,2])}" + " |"
        )
        str7 = (
            "agent 0| " + f"{add_pl(1)}" + " | " + f"{add_pl(self()[0][1,0])}" + " | "
            f"{add_pl(self()[0][1,1])}" + " | " +f"{add_pl(self()[0][1,2])}" + " |"
        )
        str8 = (
            "       | " + f"{add_pl(2)}" + " | " + f"{add_pl(self()[0][2,0])}" + " | "
            f"{add_pl(self()[0][2,1])}" + " | " +f"{add_pl(self()[0][2,2])}" + " |"
        )
        str9 = " "
        str10 = "The payoff matrix of agent 1"
        str11 = "       |  Choice agent 1"
        str12 = (
            "       | "
            + f"{add_pl(' ')}"
            + " | "
        ...
        str14 = (
            ...
            f"{add_pl(self()[1][0,1])}" + " | " + f"{add_pl(self()[1][0,2])}" + " |"
        )
        str15 = (
            "agent 0| " + f"{add_pl(1)}" + " | " + f"{add_pl(self()[1][1,0])}" + " | "
            f"{add_pl(self()[1][1,1])}" + " | " + f"{add_pl(self()[1][0,2])}" + " |"
        )
        str16 = (
            "       | " + f"{add_pl(2)}" + " | " + f"{add_pl(self()[1][2,0])}" + " | "
            f"{add_pl(self()[1][2,1])}" + " | " + f"{add_pl(self()[1][2,2])}" + " |"
        )
        str17 = str9
        return "\n".join(
            [
